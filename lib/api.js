...
var GeoPackage = require('./geoPackage')
  ...
  , VTStyle = require('./extension/vt').VTStyle;
...
GeoPackageAPI.getStylesForVectorTable = function(geopackage, tableName) {
  var layersDao = geopackage.getVTLayersDao();
  var stylesDao = geopackage.getVTStylesDao();

  var layerRows = layersDao.getRowsForTableName(tableName);
  var stylableLayerSets = new Set();
  for (var layerRow of layerRows) {
    stylableLayerSets.add(layerRow.stylable_layer_set);
  }

  var resultStyles = [];

  for (var stylableLayerSet of stylableLayerSets) {
    var stylesRows = stylesDao.getRowsForLayerSet(stylableLayerSet);
    for (var styleRow of stylesRows) {
      var vtStyle = null;
      switch(styleRow.format) {
        case 'mbstyle':
        vtStyle = VTStyle.fromMbstyle(styleRow.title || '<unnamed>', styleRow.style, styleRow.stylesheet);
        break;

        case 'sld':
        console.warn('Skipping SLD-format style; not supported yet');
        break;

        case 'cmss':
        console.warn('Skipping CMSS-format style; not supported yet');
        break;

        default:
        console.warn('Skipping unknown style format:', styleRow.format);
      }
      if (!!vtStyle) {
        resultStyles.push(vtStyle);
      }
    }
  }
